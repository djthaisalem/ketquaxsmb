...
export default function StrategyAccess({ children }) {
  ...
  if (!member) return <section className="strategy-access panel"><p className="eyebrow">KHU VỰC THÀNH VIÊN</p><h2>Mở khóa Chiến lược</h2><p>Đăng nhập để xem các công thức, kiểm thử lịch sử và quản lý gói thành viên.</p><div className="access-tabs"><button className={mode === 'login' ? 'active' : ''} onClick={() => { setMode('login'); setMessage(''); }}>Đăng nhập</button><button className={mode === 'register' ? 'active' : ''} onClick={() => { setMode('register'); setMessage(''); }}>Đăng ký</button></div><form onSubmit={submit}><label>Tên đăng nhập<input value={form.username} onChange={(event) => setForm({ ...form, username: event.target.value })} required /></label>{mode === 'register' && <label>Email<input type="email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} required /></label>}<label>Mật khẩu<input type="password" value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} minLength="10" title="Ít nhất 10 ký tự gồm chữ hoa, chữ thường, số và ký tự đặc biệt" required /></label>{mode === 'register' && <small>Mật khẩu: ít nhất 10 ký tự, gồm chữ hoa, chữ thường, số và ký tự đặc biệt.</small>}{message && <small className="access-message">{message}</small>}<button type="submit">{mode === 'login' ? 'Đăng nhập' : 'Tạo tài khoản dùng thử VIP'}</button></form></section>;
