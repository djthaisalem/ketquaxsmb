...
export default function StrategyAccess({ children }) {
  ...
  useEffect(() => {
    if (!token) return;
    api('/member/account', token).then(({ member: account }) => setMember(account)).catch(() => { localStorage.removeItem(STORAGE_KEY); setToken(''); setMessage('Phiên đăng nhập đã hết hạn. Vui lòng đăng nhập lại.'); });
  }, [token]);
  ...
  if (!member) return <><NoticePopup message={message} type="error" onClose={() => setMessage('')} /><section className="strategy-access panel"><p className="eyebrow">KHU VỰC THÀNH VIÊN</p><h2>Mở khóa Chiến lược</h2><p>Đăng nhập để xem các công thức, kiểm thử lịch sử và quản lý gói thành viên.</p><div className="access-tabs"><button className={mode === 'login' ? 'active' : ''} onClick={() => { setMode('login'); setMessage(''); }}>Đăng nhập</button><button className={mode === 'register' ? 'active' : ''} onClick={() => { setMode('register'); setMessage(''); }}>Đăng ký</button></div><form onSubmit={submit}><label>Tên đăng nhập<input value={form.username} onChange={(event) => setForm({ ...form, username: event.target.value })} required /></label>{mode === 'register' && <label>Email<input type="email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} required /></label>}<label>Mật khẩu<input type="password" value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} minLength="10" title="Ít nhất 10 ký tự gồm chữ hoa, chữ thường, số và ký tự đặc biệt" required /></label>{mode === 'register' && <small>Mật khẩu: ít nhất 10 ký tự, gồm chữ hoa, chữ thường, số và ký tự đặc biệt.</small>}<button type="submit">{mode === 'login' ? 'Đăng nhập' : 'Tạo tài khoản dùng thử VIP'}</button></form></section></>;
...
function MemberDashboard({ member, token, setMember, logout, strategy }) {
  ...
  const [notice, setNotice] = useState(() => member.trial_active ? `Đăng ký thành công: bạn được dùng thử toàn bộ chức năng VIP đến ${formatDateTime(member.trial_ends_at)}.` : '');
...
function VipHistorySummary({ days, loading, numberSize, window }) {
  ...
  return <section className="member-vip-summary"><div className="member-vip-summary-heading"><div><p className="eyebrow">KẾT QUẢ VIP ĐÃ VỀ</p><h3>Đối chiếu khung {window} ngày · VIP {numberSize} số</h3></div></div>{loading ? <p className="member-vip-empty">Đang kiểm tra kết quả thực tế…</p> : days.length ? <div className="member-vip-history">{days.map((day) => { const total = day.items.reduce((sum, item) => sum + totals(item).matched, 0); const predicted = day.items.reduce((sum, item) => sum + totals(item).predicted, 0); return <article key={day.date}><header><b>Dự báo từ {formatDate(day.date)}</b><span>{total}/{predicted || '—'} số đã về</span></header>{day.items.map((item) => { const result = totals(item); return <div key={`${item.vipMode}-${item.numberSize}`}><small>{label[item.vipMode]} · {item.numberSize} số · Khung {window} ngày · toàn bộ tín hiệu đã quét</small><strong>{item.numbers.join(' · ')}</strong><b>{result.matched}/{result.predicted || '—'} số đã về</b><section className="vip-history-window">{item.byDay.map((entry) => <span className={entry.hits ? 'hit' : ''} key={entry.day}>Ngày {entry.day} · {formatDate(entry.date)}: <b>{!entry.available ? 'chờ kết quả' : entry.hits ? `${entry.hits} lần` : 'chưa về'}</b>{entry.matched.length ? ` · ${entry.matched.join(' · ')}` : ''}{entry.evidence?.length ? <small className="vip-history-proof"> · {entry.evidence.map((proof) => `${proof.number} ← ${proof.sources.join(', ')}`).join(' | ')}</small> : null}</span>)}</section></div>; })}</article>; })}</div> : <p className="member-vip-empty">Trong các khung {window} ngày đã hoàn tất chưa có lựa chọn VIP {numberSize} số nào về.</p>}</section>;
}
