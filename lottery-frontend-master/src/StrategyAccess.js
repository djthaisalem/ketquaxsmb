...
function MemberDashboard({ member, token, setMember, logout, strategy }) {
  ...
  const [notice, setNotice] = useState(() => member.trial_active ? `Đăng ký thành công: bạn được dùng thử toàn bộ chức năng VIP đến hết ${formatDate(member.trial_ends_at)}.` : '');
