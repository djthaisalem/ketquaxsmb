...
export default function AdminApp() {
  ...
  const [vipResultRange, setVipResultRange] = useState({ from: '2026-05-01', to: new Date().toISOString().slice(0, 10), window: '3', numberSize: '2', source: 'all' });
  ...
  const content = () => {
    ...
    if (page === 'research') return <InternalBacktest report={data.research?.report} onRefresh={refreshResearch} loading={loading} referenceReport={data.research?.referenceReport} onRefreshReference={refreshReferenceResearch} referenceLoading={loading} />;
